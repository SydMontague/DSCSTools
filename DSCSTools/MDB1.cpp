...
void extractMDB1(const boost::filesystem::path source, const boost::filesystem::path target) {
	...
	auto entry1 = std::make_unique<FileEntry[]>(header.fileEntryCount);
	auto entry2 = std::make_unique<FileNameEntry[]>(header.fileNameCount);
	auto entry3 = std::make_unique<DataEntry[]>(header.dataEntryCount);

	input.read(reinterpret_cast<char*>(entry1.get()), header.fileEntryCount * sizeof(FileEntry));
	input.read(reinterpret_cast<char*>(entry2.get()), header.fileNameCount * sizeof(FileNameEntry));
	input.read(reinterpret_cast<char*>(entry3.get()), header.dataEntryCount * sizeof(DataEntry));
...
CompressionResult getFileData(const boost::filesystem::path path, const bool compress) {
	...
	if (compress) {
		...
		if (destSize + 4 < length) 
			return { (uint32_t) length, (uint32_t) destSize, std::move(outputData) };
	}

	return { (uint32_t) length, (uint32_t) length, std::move(data) };
}
