...
// TODO allow the use of pre-compressed files, e.g. by using a original archive
...
void cryptFile(const boost::filesystem::path source, const boost::filesystem::path target) {
	...
	while (!input.eof()) {
		...
		for (int i = 0; i < count; i++) {
			outArr[i] = inArr[i] ^ key1[key1Offset] ^ key2[key2Offset];
